...
class RouteScheduleHandler {
  ...
  updateBusLocations() {
    ...
    const activeBuses = route.schedule.filter((schedule) => {
      ...
      return currentTime >= this.adjustTime(startTime, -2) && currentTime <= endTime;
    });
    ...
    this.busLocations = activeBuses.map((bus) => {
      ...
      for (let i = 0; i < times.length; i++) {
        if (currentTime >= this.adjustTime(times[i], -2)) {
          currentStopIndex = i;
        }
      }
  ...
  updateArrowDirection() {
    const busIcons = document.querySelectorAll('.bus-icon .bi-arrow-90deg-up');
    busIcons.forEach(icon => {
      icon.classList.remove('bi-arrow-90deg-up');
      icon.classList.add('bi-arrow-90deg-down');
    });
  }
  ...
  updateStopsDisplay() {
    ...
    // 更新第三個站點的箭頭方向
    this.updateThirdStopArrowDirection();
  }

  updateThirdStopArrowDirection() {
    const busIcons = document.querySelectorAll('.station-container:nth-child(3) .bus-icon .bi-arrow-90deg-down');
    busIcons.forEach(icon => {
      icon.classList.remove('bi-arrow-90deg-down');
      icon.classList.add("bi-arrow-bar-up");
    });

    // 隱藏回到第一個站點的圖標
    // const firstStopIcons = document.querySelectorAll('.station-container:first-child .bus-icon');
    // firstStopIcons.forEach(icon => {
