  ...
  updateBusLocations() {
    ...
    this.busLocations = activeBuses.map((bus) => {
      ...
      for (let i = 0; i < times.length; i++) {
        const arrivalTime = times[i];
        const departureTime = this.adjustTime(arrivalTime, 0.5); // 30秒後離開

        if (currentTime >= this.adjustTime(arrivalTime, -2)) {
          currentStopIndex = i;
          hasArrived = currentTime < departureTime;
        }
      }
          ...
          <div class="ms-5">
            ${busesAtStop
              ...
              <div class="bus-icon">
                <i class="bi ${
                  index === 0 ? 'invisible' :
                  index === 2 ? 'bi-arrow-bar-up' :
                  bus.hasArrived ? 'bi-arrow-90deg-up' : 'bi-arrow-90deg-down'
                } animated-hover"></i>
